...
## Overview
A command line calculator built from scratch that supports direct expression evaluation, an interactive mode, and a history feature. The calculator uses 'Pratt Parsing' for parsing expressions and features a custom pool allocator (Arena) for memory management. It can also be used as a library—simply ignore the `main.c` file.
